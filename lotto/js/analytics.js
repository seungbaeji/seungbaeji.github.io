...
class LottoAnalytics {
    ...
    getConsecutivePattern(startDraw = null, endDraw = null) {
        ...
        draws.forEach(draw => {
            ...
            const key = consecutiveCount === 0 ? '0개' :
                       consecutiveCount === 1 ? '1개' :
                       consecutiveCount === 2 ? '2개' :
                       consecutiveCount === 3 ? '3개' : '4개+';
            patterns[key]++;
        });
    ...
    checkCombinationExists(numbers) {
        const combination = numbers.slice().sort((a, b) => a - b).join(',');
